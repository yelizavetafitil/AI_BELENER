...
_SNIP_BARE_NUM = re.compile(
    r"(?<![\d.])(?P<num>\d{1,2}\.\d{2}\.\d{2}-\d{2,4})(?![\d.])"
)
_SNIP_CONTEXT_HINT = re.compile(
    r"(?i)устройства|требования|соответствии|согласно|электротехн|нормам|снип|\(|\[",
)
...
_CLIP: dict[str, re.Pattern[str]] = {
    ...
    "СТП": re.compile(r"^(\d+(?:[\s.]\d+)+(?:-\d{2,4})?)", re.I),
    "РД": re.compile(r"^(\d+(?:[\s.]\d+)+(?:-\d{2,4})?)", re.I),
...
def _num_complete(num: str, kind: str) -> bool:
    ...
    if kind in ("СП", "СН"):
        # СП/СН 1.03.11-2023; СП 63.13330.2018; отсечь «СП 1202-».
        compact = n.replace(" ", "")
        if re.fullmatch(r"\d+(?:\.\d+){1,}-\d{2,4}", compact):
            return True
        if re.fullmatch(r"\d+(?:[-–—]\d+){1,}-\d{2,4}", compact):
            return True
        # год без дефиса: 63.13330.2018
        if re.fullmatch(r"\d+(?:\.\d+){2,}", compact) and re.search(
            r"\.(?:19|20)\d{2}$|\.\d{2}$", compact
        ):
            return True
        return False
    if kind == "НРР":
        compact = n.replace(" ", "")
        return bool(
            re.fullmatch(r"\d+(?:\.\d+){1,}-\d{2,4}", compact)
            or (
                re.fullmatch(r"\d+(?:\.\d+){2,}", compact)
    ...
    if kind_m:
        ...
        if num:
            ...
            if kind == "ОСТ":
                ...
                add(f"RD {num_fmt}")
                add(f"RD{num_fmt}", require_space_after_kind=False)
                add(f"(RD {num_fmt})")
                add(f"(RD{num_fmt})", require_space_after_kind=False)
            elif kind == "СТП":
                add(f"STP {num_fmt}")
                add(f"STP{num_fmt}", require_space_after_kind=False)
                add(f"(STP {num_fmt})")
            elif kind == "СНиП":
                add(f"SNIP {num_fmt}")
                add(f"SNIP{num_fmt}", require_space_after_kind=False)
            if kind in ("СТП", "РД", "СНиП"):
                body, year = _body_year_digits(kind, f"{kind} {rest}")
                if year and len(year) >= 2:
                    num_base = format_stp_number(body) if kind in ("СТП", "РД") else body
                    add(f"{kind} {num_base}")
                    add(f"({kind} {num_fmt})")
...
def _normalize_kind_label(raw: str) -> str:
    """Единая подпись типа: GOST→ГОСТ, SNIP→СНиП, .upper() не ломает «СНиП»."""
    t = (raw or "").strip()
    if not t:
        return ""
    ...
    if compact in by_fold:
        return by_fold[compact]
    upper = t.upper()
    by_upper = {
        "GOST": "ГОСТ",
        "OST": "ОСТ",
...
def _ref_highlight_target(ref: str) -> tuple[str, str, str]:
    ...
    kind = _normalize_kind_label(kind_m.group(1)) if kind_m else ""
...
def _normalize_highlight_phrase(phrase: str, kind: str) -> str:
    """OCR-подмены типа FOCT→GOST, TKN→ТКП — только для подсветки на сканах."""
    s = phrase or ""
    per_kind: dict[str, list[tuple[str, str]]] = {
        "ГОСТ": [
            (r"(?<![a-zа-яё])(?:FOCT|TOCT|GOCT|G0ST|fOCT|\[OCT|\(OCT)(?![a-zа-яё])", "GOST"),
        ],
        "ОСТ": [
            (r"(?<![a-zа-яё])(?:\[OCT|\(OCT)(?![a-zа-яё])", "ОСТ"),
        ],
        "ТКП": [
            (r"(?<![a-zа-яё])TKN(?![a-zа-яё])", "ТКП"),
        ],
        "СТБ": [
            (r"(?<![a-zа-яё])(?:CTB|CT5|CTE|CIb|CIB)(?![a-zа-яё])", "СТБ"),
        ],
        "СТП": [
            (r"(?<![a-zа-яё])(?:CTN|STN|\(СТП|\(STP)(?![a-zа-яё])", "СТП"),
        ],
        "РД": [
            (r"(?<![a-zа-яё])(?:\(РД|\(RD|PA)(?![a-zа-яё])", "РД"),
        ],
        "СНиП": [
            (r"(?<![a-zа-яё])(?:CHN|CHiP|CHИП|СНiП)(?![a-zа-яё])", "СНиП"),
        ],
    }
    for pat, repl in per_kind.get(kind, []):
        s = re.sub(pat, repl, s, flags=re.I)
    return s


def _highlight_kind_re(kind: str) -> str:
    return {
        "ГОСТ": r"гост|gost|goct|foct|toct|g0st|\[?oct|\(?oct",
        "ОСТ": r"ост|ost|\[?oct|\(?oct",
        "ТУ": r"ту|tu",
        "СТБ": r"стб|stb|ctb|ct5|cte|cib",
        "СТП": r"стп|stp|ctn|stn|\(?stp",
        "ТКП": r"ткп|tkp|tkn",
        "СНиП": r"снип|snip|chn|\(?snip",
        "СН": r"сн|ch",
        "НРР": r"нрр|hrr|nrr",
        "СП": r"сп|sp",
        "РД": r"рд|rd|pa|\(?rd",
...
def word_looks_like_kind_token(text: str, kind: str) -> bool:
    t = (text or "").strip()
    if not t or not kind:
        return False
    aliases = {
        "ГОСТ": frozenset({"гост", "gost", "goct", "foct", "toct", "g0st"}),
        "ОСТ": frozenset({"ост", "ost", "oct"}),
        "ТУ": frozenset({"ту", "tu"}),
        "СТБ": frozenset({"стб", "stb", "ctb", "ct5", "cte", "cib"}),
        "СТП": frozenset({"стп", "stp", "ctn", "stn"}),
        "ТКП": frozenset({"ткп", "tkp", "tkn"}),
    ...
    bare = re.sub(r"^[\[\(]+|[\]\)]+$", "", t).casefold()
    if bare in aliases.get(kind, frozenset({kind.casefold()})):
        return True
    return bool(re.match(rf"(?:[\[\(])?{_highlight_kind_re(kind)}(?:[\]\)])?$", t, re.I))
...
def _phrase_matches_highlight_ref(
    ...
    *,
    ...
    if not blob or not kind:
        return False
    kind_re = _highlight_kind_re(kind)
...
def format_stp_number(num: str) -> str:
    ...
    year_suffix = ym.group(0) if ym else ""
    body = s[: ym.start()] if ym else s
    compact = re.sub(r"[\s.]", "", body)
    m = re.match(r"^34(\d{2})(\d{3})$", compact)
    if m:
        body = f"34.{m.group(1)}.{m.group(2)}"
    else:
        prev = None
        body = body
        while prev != body:
...
def merge_normative_refs_from_sources(*source_texts: str) -> list[dict[str, str]]:
    ...
    for key in order_keys:
        ...
        best_ref = max(
            by_ref.keys(),
            key=lambda r: (
                len(_body_year_digits(kind, r)[1]),
    ...
    for item in merged:
        ...
        ref = str(item.get("ref") or "")
        if kind == "ГОСТ":
            ref = _repair_gost_ref_from_sources(kind, ref, uniq)
        elif kind in ("СТП", "РД", "СНиП"):
            ref = _repair_dotted_ref_from_sources(kind, ref, uniq)
        repaired.append({**item, "ref": ref})
    return dedupe_normative_list(repaired)


def _repair_gost_ref_from_sources(kind: str, ref: str, sources: list[str]) -> str:
    """Выбрать лучший вариант ГОСТ с тем же телом номера по голосам тайлов."""
    if kind != "ГОСТ" or not ref or not sources:
        return ref
    body, _year = _body_year_digits(kind, ref)
    if not body or len(body) < 3:
        return ref
    pool: list[dict[str, str]] = []
    for src in sources:
        pool.extend(extract_normative_refs(src))
    pool.extend(extract_normative_refs("\n\n".join(sources)))
    same_body: list[dict[str, str]] = []
    for item in pool:
        if str(item.get("kind") or "") != "ГОСТ":
            continue
        ib, _ = _body_year_digits("ГОСТ", str(item.get("ref") or ""))
        if ib == body:
            same_body.append(item)
    if not same_body:
        blob = _ocr_loosen_normative_spacing("\n".join(sources))
        for m in re.finditer(rf"(?i)(?:гост|gost)\s*([\d\.]+)-(\d{{4}})\b", blob):
            if re.sub(r"\D", "", m.group(1)) != body:
                continue
            num = _clip_num(m.group(1), "ГОСТ")
            if num and _num_complete(f"{num}-{m.group(2)}", "ГОСТ"):
                return _normalize_gost_ref_display(f"ГОСТ {format_gost_number(f'{num}-{m.group(2)}')}")
        return _normalize_gost_ref_display(ref)
    best = max(
        same_body,
        key=lambda c: (
            _ref_vote_count("ГОСТ", str(c.get("ref") or ""), sources),
            _gost_variant_quality(str(c.get("ref") or "")),
            len(_body_year_digits("ГОСТ", str(c.get("ref") or ""))[1]),
            len(_light_clean(str(c.get("ref") or ""))),
        ),
    )
    return _normalize_gost_ref_display(str(best.get("ref") or ref))


def _repair_dotted_ref_from_sources(kind: str, ref: str, sources: list[str]) -> str:
    """СТП/РД/СНиП: выбрать вариант с полным годом (-2018, -87) по голосам тайлов."""
    if kind not in ("СТП", "РД", "СНиП") or not ref or not sources:
        return ref
    body, year = _body_year_digits(kind, ref)
    if not body:
        return ref
    ...
    same_body = [
        ...
        and _body_year_digits(kind, str(item.get("ref") or ""))[0] == body
    ]
    if not same_body:
        blob = _ocr_loosen_normative_spacing("\n".join(sources))
        kind_re = {"СТП": r"стп|stp", "РД": r"рд|rd", "СНиП": r"снип|snip"}[kind]
        best_blob: str | None = None
        for m in re.finditer(
            rf"(?i)(?:{kind_re})\s*([\d\.]+)-(\d{{4}}|\d{{2}})\b",
            blob,
        ):
            if re.sub(r"\D", "", m.group(1)) != body:
                continue
            yr = m.group(2)
            num = format_stp_number(f"{m.group(1)}-{yr}") if kind in ("СТП", "РД") else f"{m.group(1)}-{yr}"
            candidate = f"{kind} {num}"
            if _num_complete(num, kind):
                if best_blob is None or len(yr) > len(_body_year_digits(kind, best_blob)[1]):
                    best_blob = candidate
        if best_blob:
            return _sanitize_normative_ref(best_blob)
        return ref
    best = max(
        same_body,
        key=lambda c: (
            len(_body_year_digits(kind, str(c.get("ref") or ""))[1]),
            _ref_vote_count(kind, str(c.get("ref") or ""), sources),
            len(_light_clean(str(c.get("ref") or ""))),
        ),
    )
    out = str(best.get("ref") or ref)
    if kind in ("СТП", "РД"):
        out = _sanitize_normative_ref(out)
        m = re.match(rf"^(?:{kind})\s+(.+)$", _light_clean(out), re.I)
        if m:
            out = f"{kind} {format_stp_number(m.group(1))}"
    return out


def _flatten_gost_ocr_dot(num: str) -> str:
    """9.032-74 → 9032-74; серию 9.402 не трогаем."""
    m = re.match(r"^(\d)\.(\d{2,})(.*)$", (num or "").strip())
    if not m:
        return num
    if m.group(2).startswith("0") and len(m.group(2)) >= 3:
        return f"{m.group(1)}{m.group(2)}{m.group(3)}"
    return num


def _normalize_gost_ref_display(ref: str) -> str:
    m = re.match(r"^(?:(?:ГОСТ|GOST))\s*(.+)$", _light_clean(ref), re.I)
    if not m:
        return ref
    num = _flatten_gost_ocr_dot(m.group(1).strip())
    if _num_complete(num, "ГОСТ"):
        return f"ГОСТ {format_gost_number(num)}"
    return ref
...
def _canonical_key(kind: str, ref: str) -> str:
    ...
    if kind == "ОСТ":
        ...
        if digits:
            return f"{kind.casefold()}:{digits}"
    if kind in ("ГОСТ", "ТУ", "СТБ", "СНиП", "ТКП", "СП", "СН", "НРР"):
        digits = _canonical_number(kind, ref)
        if digits:
            return f"{kind.casefold()}:{digits}"
    num_m = re.search(
...
def _ocr_loosen_normative_spacing(text: str) -> str:
    ...
    s = re.sub(
        r"(?i)(требованиями|согласно|в\s+соответствии\s+с)\s+(СНиП|SNIP)\s+(\d[\d\.]+-\d{2,4})",
        r"\2 \3",
        s,
    )
    ...
    s = re.sub(
        ...
        r"СНиП|SNIP|СН|CH|НРР|HRR|NRR|ТКП|TKP|СП|SP|DIN|EN|ISO|IEC))\s*\n+\s*([\d\+])",
        r"\1 \2",
        s,
        flags=re.I,
    )
...
def _is_snip_bare_number(num: str) -> bool:
    return bool(re.fullmatch(r"\d{1,2}\.\d{2}\.\d{2}-\d{2,4}", _light_clean(num)))


def _snip_bare_context_ok(text: str, start: int, end: int, num: str) -> bool:
    win = _window(text, start, end, radius=90)
    if is_noise_span(win, num) or not _SNIP_CONTEXT_HINT.search(win):
        return False
    before = text[max(0, start - 8) : start]
    if re.search(r"(?i)(?:снип|snip)\s*$", before):
        return False
    return accept_by_context(win, num, prefix="СНиП")


def _extract_implicit_snip_refs(text: str) -> list[dict[str, str]]:
    """СНиП без префикса в заметках: (3.05.06-85) после «устройства» и т.п."""
    out: list[dict[str, str]] = []
    seen: set[str] = set()
    for m in _SNIP_BARE_NUM.finditer(text):
        num = _light_clean(m.group("num"))
        if not _is_snip_bare_number(num) or not _num_complete(num, "СНиП"):
            continue
        before = text[max(0, m.start() - 12) : m.start()]
        # Уже есть явный СН/СП/НРР/СНиП перед номером — не дублировать как «голый» СНиП.
        if re.search(
            r"(?i)(?:снип|snip|нрр|hrr|nrr|сн|ch|сп|sp)\s*$",
            before.rstrip(),
        ):
            continue
        if not _snip_bare_context_ok(text, m.start(), m.end(), num):
            continue
        num = format_stp_number(num)
        ref = _sanitize_normative_ref(f"СНиП {num}")
        if not ref:
            continue
        key = _canonical_key("СНиП", ref)
        if key in seen:
            continue
        seen.add(key)
        ctx = re.sub(r"\s+", " ", _window(text, m.start(), m.end()).strip())[:160]
        out.append({"kind": "СНиП", "ref": ref, "context": ctx or ref})
    return out
...
def _snip_looks_like_modern_sp(ref: str) -> bool:
    """OCR часто путает СП и СНиП: СП 1.03.04-2020 → СНиП 1.03.04-2020."""
    ...
    if len(year) == 4:
        try:
            ...
            return False
    # Двухзначный год: 10–39 — усечение 2010–2039 (СП); 70–99 — классический СНиП.
    try:
        yi = int(year)
    except ValueError:
        return False
